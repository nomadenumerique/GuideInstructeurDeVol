<center><h1>Exercice 18: Approche et atterrissage</h1></CENTER>

<FONT SIZE=1 FACE="Geneva">Plans de le&ccedil;ons 3, 4, 7



D&eacute;finition:
manoeuvre qui permet &agrave; l'avion de prendre contact avec
le sol &agrave; la vitesse verticale la plus basse possible, 
&agrave; la vitesse horizontale la plus basse possible compatible
avec un contr&ocirc;le suffisant de l'avion, et avec les roues
du train d'atterrissage principal align&eacute;es avec l'axe de
piste.



Objectifs:
+ techniques pour poser l'avion dans diverses conditions

+ d&eacute;termination de l'approche &agrave;
utiliser



## Motivation :
Bien que les techniques d'approche et d'atterrissage ne soient
pas plus importantes ou moins importantes que les autres techniques
de pilotage, la r&eacute;ussite d'un bel atterrissage apporte
au pilote une grande satisfaction.



R&eacute;f&eacute;rences :
GIV 123

MP 117

MV 194-197;  202-205

PPI 77-85;  85-89

PPII 71-77;  77-84

S&amp;R 262-286;  287-305;  318-319

AIP AIR  2.7

2.13

Van Sickle 408;  419-423;  423-430

Ann. FAA Test Stds  220-247

AOPA Flt Training  13-8 &agrave; 13-29;
 13-38 &agrave; 13-68

Kershner SPFM 99-103; 106-111; 114-122;
149-156;

285-286

Kershner APM  112-120;  200-202;  263-265

Kershner FIM  45-47;  82-86;  90-92;  95-97;
 100-103;     114-116; 117-120; 187-190



Au sol:

<B>Aides:</B>+
maquette d'avion

+ Manuel de l'avion  1-5 &agrave; 1-6; 
2-15 &agrave; 2-16;  6-3

POH  4-3;  4-8 &agrave; 4-9;  4-19 &agrave;
4-20; 5-7;  5-19


<B>Points &agrave; d&eacute;montrer :</B>

+ Manuel d'op&eacute;ration de l'avion (POH)
:

- configurations d'atterrissage (POH 4-19
&agrave; 4-20)

- tables de distances d'atterrissage <U>selon
techniques</U>          <U>d'atterrissage court </U>(POH 5-19)

+ facteurs influen&ccedil;ant la longueur
de la course d'atterrissage 

- altitude-densit&eacute;, (vitesse) et
poids (&eacute;nergie cin&eacute;tique)

- ajouter 4% /1000' d'alt-densit&eacute;
(1000' d'alt-press; 8.5&#176;C)

- Ek =  mV<FONT SIZE=1 FACE="Geneva">2
/2

roulage (en pi) =  0.225 V<FONT SIZE=1 FACE="Geneva">2
(mph)   ou   0.3 V<FONT SIZE=1 FACE="Geneva">2
(kts)  

- directement proportionnelle au poids
&agrave; l'atterrissage

- vent :

- de face :  90% - (comp. vent de face/Vr)%
= (dist. d&eacute;c.)%

- de travers

- instable en direction ou arri&egrave;re

- pente de la piste

- &eacute;tat de la surface (en dur, en
herbe, mouill&eacute;e, enneig&eacute;e, glac&eacute;e)

- mauvaise technique du pilote : vit. excessive,
<U>freinage</U> excessif

+ revoir exercice 8  (Descentes)

+ d&eacute;termination du genre d'<B>approche</B>
&agrave; effectuer :

- obstacle

- visibilit&eacute; et rafales

- turbulence de sillage

- longueur du terrain

- &eacute;tat de la surface du terrain

- genre d'atterrissage

+ d&eacute;termination du genre d'<B>atterrissage</B>
&agrave; effectuer :

- force et direction du vent

- longueur du terrain

- &eacute;tat de la surface du terrain

+ 2 diff&eacute;rents genres d'approche :

- &agrave; pente et vitesse d'approche normales
 (sans moteur et avec)

- &agrave;  vitesse d'approche r&eacute;duite
 (avec moteur seul.)

+ juger l'approche :

- approche trop haute :

_ indices visuels :  le point de contact
semble descendre ;

l'avion n'est pas au sol avant un certain
point sur la piste

_ causes :

- virage en vent arri&egrave;re trop t&ocirc;t

- &ecirc;tre distrait et monter au-dessus
de 1000' AGL

- r&eacute;duction initiale de puissance
insuffisante

- virage en base trop t&ocirc;t

- sortir les volets trop peu et/ou trop
tard

- oublier de continuer &agrave; r&eacute;duire
la puissance

- virage en finale trop t&ocirc;t  (aussi
sera mal align&eacute;)

_ consid&eacute;rations sur la remont&eacute;e
    (voir ci-bas)

- approche trop basse :

_ indices visuels :  le point de contact
ou les obstacles       semblent monter

_ causes :  &ecirc;tre en arri&egrave;re
de l'avion et virer trop tard en     vent arri&egrave;re, ou en
base

r&eacute;duction initiale de puissance
excessive

_ mani&egrave;re d'ajouter la puissance
(quand, combien, pourquoi)

_ transformation d'une approche basse en
approche haute       et/ou trop rapide et n&eacute;cessit&eacute;
d'effectuer une remont&eacute;e

+ indices visuels permettant de juger l'atterrissage
:

- garder le regard d&eacute;tendu

- regarder tout le panorama (hangars, avions
stationn&eacute;s, arbres,          herbe, horizon,....)

- pendant l'approche:  emp&ecirc;cher les
marques de <B>seuil</B> de piste de    &#171;bouger&#187; par
rapport au capot jusqu'au d&eacute;but de l'arrondi    (maintenir
vitesse constante avec gouv. de profondeur,     contr&ocirc;ler
taux de descente avec volets et puissance)

- d&eacute;but de l'arrondi :

effectuer rapidement (1) et (2) :

1) la hauteur &agrave; laquelle l'arrondi
<U>d&eacute;bute</U> et la <U>cadence</U> &agrave;    laquelle
il est effectu&eacute;, d&eacute;pendent du taux de descente 
   ant&eacute;rieur  (regarder &agrave; quelle vitesse les choses,
surtout    <B>proches du seuil</B>, &#171;bougent&#187;)

2) d&eacute;termination de la hauteur en
regardant sous quelle     perspective se pr&eacute;sentent des
objets familiers      (regarder surtout o&ugrave; <B>l'horizon
</B>&#171;coupe&#187; les objets)

3) diriger le regard <U>&agrave; gauche</U>
le long du capot, &agrave; la m&ecirc;me    distance qu'habituellement
d'une auto roulant &agrave; la m&ecirc;me    vitesse, c-a-d. <U>assez</U>
<U>loin</U> pour que le sol ne soit pas flou et    <U>assez pr&egrave;s</U>
pour pouvoir d&eacute;tecter les changements de     hauteur  
 (<U>environ</U>  <B>&agrave; mi-piste )</B>

- apr&egrave;s que la descente ait &eacute;t&eacute;
arr&ecirc;t&eacute;e  (en palier &agrave; environ 1') :

- une et une seule assiette pour chaque
hauteur au-dessus     de la piste

- d&eacute;terminer si l'avion s'enfonce
ou s'&eacute;l&egrave;ve le moindrement

(au sol,  s'exercer &agrave; d&eacute;tecter
les changements de     perspective en s'&eacute;levant sur le
bout des pieds)

- si l'avion s'&eacute;l&egrave;ve, simplement
attendre, garder la main     fixe, laisser le ballonnement se
dissipper par soi-    m&ecirc;me, puis recommencer &agrave; tirer
sur le manche      lorsque l'avion commence &agrave; s'enfoncer

- si l'avion s'enfonce, augmenter la pression
arri&egrave;re

- la vit. &agrave; laquelle on applique
la pression arri&egrave;re s'acc&eacute;-   l&egrave;re,  au fur
et &agrave; mesure que la vitesse de l'avion diminue

- v&eacute;rifier la direction <U>et l'inclinaison
lat&eacute;rale</U> au-dessus    de la piste, du coin de l'oeil,
et corriger avec les palonniers


+ 4 diff&eacute;rentes sortes d'atterrissage
:

- normal

- par vent de travers :

APPROCHE EN CRABE ET ATTERRISSAGE AILE BASSE
:

- pendant l'approche, corriger la d&eacute;rive
en maintenant un &#171;angle   de crabe&#187;

- s'il y a des rafales, ajouter &agrave;
la vitesse d'approche 1/2 gust

- au d&eacute;but de l'arrondi, adopter
et maintenir une glissade lat&eacute;rale   suffisante pour emp&ecirc;cher
tout d&eacute;placement lat&eacute;ral

- si le vent de travers est fort ou par
rafales,  prendre contact   avec la piste &agrave; une vitesse
sup&eacute;rieure &agrave; la normale pour conserver   l'efficacit&eacute;
du gouv. de direction et r&eacute;duire l'angle d'attaque (ne
  pas garder la roue en l'air apr&egrave;s s'&ecirc;tre pos&eacute;)

- pendant le cabrage final, le nez de l'avion
(l'axe longitudinal)   semblera prendre un angle avec l'axe de
piste du c&ocirc;t&eacute; oppos&eacute; au   vent &agrave; cause
de l'assiette simultan&eacute;ment cabr&eacute;e et inclin&eacute;e
   (d&eacute;montrer avec maquette)

- impact sur la roue principale au vent

- l'efficacit&eacute; des ailerons diminuant,
l'autre roue principale    touchera la piste

APPROCHE AILE BASSE ET ATTERRISSAGE AILE
BASSE :

- comme ci-dessus,  sauf que la d&eacute;rive
est corrig&eacute;e avec une    glissade lat&eacute;rale pendant
toute l'approche

-  courts  (<U>et </U>obstacle de 50') :

- approche <U>au moteur (</U>&agrave; vitesse
r&eacute;duite): &lt;1.3 Vso(CAS) + 1/2gust

2 raisons : _ vit. r&eacute;duite :  flottement
r&eacute;duit

_ meilleur contr&ocirc;le de la pente

- faire un circuit plus large :

_ pour &eacute;viter de virer excessivement
inclin&eacute; (20&#176; max) &agrave; <U>basse</U>    <U>vitesse</U>

_ la pente de l'approche au moteur est moindre
:  besoin de plus    d'espace pour descendre

- au d&eacute;but de la finale :  pleins
volets et vitesse stabilis&eacute;e

-  contr&ocirc;le de la vitesse avec le
manche

contr&ocirc;le de la pente avec la puissance

- garder de la puissance tant que l'avion
n'est pas &eacute;tabli en palier    au-dessus de la piste  (i.e.
tant que l'arrondi n'est pas fini)

- &eacute;viter de prolonger le flottement
au-dessus de la piste      (couper compl&egrave;tement moteur
et placer l'avion au sol)

- apr&egrave;s que l'avion a pris contact
avec le sol, diminuer l'assiette,    rentrer les volets (pas les
roues !), tenir le manche compl&egrave;tement   en arri&egrave;re
et freiner au maximum sans bloquer les roues

- sur terrain mou ou cahoteux :

- reconnaissance du terrain  (approche de
pr&eacute;caution :  ex. 21)

- approche finale de terrain court (plus
de pr&eacute;cision &agrave; la prise de            contact)

- garder de la puissance pendant tout l'arrondi
<U>et</U> jusqu'&agrave; ce que    <U>toutes les roues</U> de
l'avion aient pris contact avec le sol,    manche &agrave; fond
en arri&egrave;re  (vit. min. de contact  roue avant)

- prendre contact &agrave; la plus faible
vitesse verticale et horizontale    possible

- ne pas utiliser les freins  (continuer
&agrave; rouler jusqu'&agrave; &ecirc;tre sur   terrain ferme)



+ brouettage

+ marsouinage

+ turbulence de sillage

+ comment et quand le pilote doit refuser
l'autorisation d'atterrissage

+ remise de gaz (&#171;go-around&#187;, &#171;overshoot&#187;)
:

Raisons :

- approche trop haute

- d&eacute;passement de l'axe de piste en
finale (virage serr&eacute;)

- avion, v&eacute;hicule ou animal sur la
piste

- trop pr&egrave;s derri&egrave;re avion
en finale

- possibilit&eacute; de turb. de sillage
pendant la finale ou sur la piste

- instruction ATC

- conditions trop s&eacute;v&egrave;res
pour exp&eacute;rience du pilote  (turbulence,   pluie abondante,
vent de travers excessif, ...)

- approche non stabilis&eacute;e

- vitesse trop haute ou trop basse

- apr&egrave;s un ballonnement excessif
ou un rebond important

- sentiment que &#171;quelque chose&#187;
ne va pas pendant l'approche

- &ecirc;tre mal &agrave; l'aise mentalement
ou physiquement

Jugement &agrave; d&eacute;velopper  :

- prendre la d&eacute;cision <U>t&ocirc;t</U>

- IMP :  une fois la puissance ajout&eacute;e,
ne jamais changer d'id&eacute;e

Technique :

- assiette,  puissance,  r&eacute;ch-carb

- enlever dernier cran de volets

- trajectoire

+ pos&eacute;-d&eacute;coll&eacute; :

- contr&ocirc;le en direction

- utilisation des commandes auxiliaires

- piste restante suffisante

+ s&eacute;curit&eacute; sur la piste :

- virages &agrave; vitesse r&eacute;duite

- d&eacute;gagement de la piste en service

- v&eacute;rifications apr&egrave;s atterrissage

- autorisation de roulage

+ illusions d'optique en approche :

- piste en mont&eacute;e ou en descente

- piste plus large ou plus &eacute;troite
qu'&agrave; l'habitude

+ indications des instruments 



Discipline a&eacute;ronautique :

Pendant l'approche et l'atterrissage :

- voler une approche stabilis&eacute;e, sur
la bonne pente, dans l'axe et &agrave; la  bonne vitesse.  Maintenir
un contr&ocirc;le serr&eacute;, ferme et positif sur ces  trois
variables, avec une main sur le manche et l'autre sur la manette
 des gaz (gouv. de profondeur contr&ocirc;le vitesse et puissance
contr&ocirc;le taux  de descente)

- utiliser les volets par &eacute;tapes de
10&#176; et compenser apr&egrave;s chaque &eacute;tape

- lorsque l'atterrissage est compl&eacute;t&eacute;,
circuler en dehors de la piste &agrave;  vitesse mod&eacute;r&eacute;e,
d&egrave;s que possible, pour ne pas d&eacute;ranger inutilement
les  autres avions

Pendant la remont&eacute;e :

- prendre positivement la d&eacute;cision
d'interrompre l'approche et  effectuer la remont&eacute;e sans
h&eacute;sitation.  Une remont&eacute;e n'est PAS un signe  d'&eacute;chec
:  cela peut &ecirc;tre le signe d'une excellente discipline 
a&eacute;ronautique

- exercer un contr&ocirc;le ferme, positif
et souple sur l'avion.  On doit  appliquer une pression ferme
sur le manche et sur les p&egrave;dales lors de  l'application
de la pleine puissance  (re-compenser d&egrave;s que possible)

- s'assurer d'&ecirc;tre en haut de Vs<B>1</B>
avant de remonter les volets

(voir POH)

- garder en vue l'avion encore sur la piste
en prenant une trajectoire  d&eacute;cal&eacute;e par rapport
&agrave; la piste

- une fois &eacute;tabli <U>confortablement</U>
dans la remont&eacute;e, aviser la tour que  nous sommes sur la
remont&eacute;e



R&eacute;capitulation :

Erreurs courantes qu'un &eacute;l&egrave;ve
peut commettre lorsqu'il ou elle essaye de corriger une <B>approche
trop basse</B> :

- ne pas voir suffisament &agrave; l'avance
que l'avion va se trouver trop bas

- application timide de la puissance

- applications et fermetures brusques de
la puissance :  probl&egrave;mes de   vit. et alt. et de changements
d'assiette

- essayer d'&eacute;tirer la descente sans
ajouter de puissance


Erreurs fr&eacute;quentes lors de la pratique
des <B>atterrissages normaux</B> :

- position ou poids de l'avant-bras applique
involontairement de  l'aileron

- trop se concentrer sur l'assiette en tangage
et n&eacute;gliger le contr&ocirc;le en  direction et lat&eacute;ral

- regarder trop pr&egrave;s :  arrondi trop
haut

- regarder trop loin :  descend dans la piste

- essayer de regarder en avant par-dessus
le capot :

_ prise de contact pas suffisament cabr&eacute;e
et rebondit

_ tire m&eacute;caniquement sur le manche
sans rien voir

- ne pas garder la main sur la puissance

- ne pas maintenir l&eacute;g&egrave;re pression
arri&egrave;re pendant le roulage


Erreurs fr&eacute;quentes lors des atterrissages
<B>par rafales de vent</B> :

- plaquer brutalement l'avion au sol  (dommages
&agrave; la roue de nez)

- ne pas prendre les rafales en consid&eacute;ration
et garder la roue de nez en  l'air apr&egrave;s l'atterrissage



Erreurs fr&eacute;quentes lors des atterrissages
<B>par vent</B> <B>de travers</B> :

APPROCHE EN CRABE ET ATTERRISSAGE AILE BASSE
:

- transition confuse du crabe &agrave; la
mise d'aile basse

APPROCHE AILE BASSE ET ATTERRISSAGE AILE BASSE
:

- laisser l'avion tourner lorsqu'on baisse
l'aile

- ne pas assez corriger la d&eacute;rive
lat&eacute;rale

- ne pas avoir le capot align&eacute; avec
la piste (trop de palonnier ext&eacute;rieur):

approcher en glissade en ligne droite

- changer constamment l'inclinaison et la
correction en lacet, sans   n&eacute;cessit&eacute; (essayer de
corriger la d&eacute;rive m&eacute;caniquement, sans l'estimer
 d'abord)

- laisser l'inclinaison diminuer pendant
l'atterrissage, ou ne pas  l'augmenter pour compenser la diminution
de vitesse de l'avion

- essayer de remonter l'aile avant la prise
de contact   (il n'y a aucun  probl&egrave;me &agrave; atterrir
sur une roue)

- &ecirc;tre si concentr&eacute; sur la correction
de la d&eacute;rive que l'atterrissage est  n&eacute;glig&eacute;



Erreurs fr&eacute;quentes lors de la pratique
des <B>atterrissages courts</B> :

- laisser la vitesse augmenter pendant l'approche

- circuit trop serr&eacute;  (ne laisse pas
le temps de stabiliser l'approche)

- mauvais contr&ocirc;le de la pente de descente
 (trop de puissance au d&eacute;but  de l'approche et la puissance
doit &ecirc;tre compl&egrave;tement coup&eacute;e plus tard.

<U>IMP</U>: Une <U>remont&eacute;e s'impose</U>
si cette correction tardive est insuffisante.

- si l'avion arrive trop vite, tendance &agrave;
essayer de plaquer brutalement  l'avion au sol, au lieu d'effectuer
une remont&eacute;e

- mauvaise technique de freinage: bloquer
les roues, application in&eacute;gale


Erreurs fr&eacute;quentes lors de la pratique
des <B>atterrissages sur terrain mou</B> :

- laisser la vitesse augmenter pendant l'approche

- circuit trop serr&eacute;  (ne laisse pas
le temps de stabiliser l'approche)

- mauvais contr&ocirc;le de la pente de descente
 (trop de puissance au d&eacute;but  de l'approche et la puissance
doit &ecirc;tre compl&egrave;tement coup&eacute;e plus tard.

<U>IMP</U>: Une <U>remont&eacute;e s'impose</U>
si cette correction tardive est insuffisante.

- arrondi trop haut

- ne pas tenir le manche compl&egrave;tement
en arri&egrave;re apr&egrave;s l'atterrissage

- mauvais contr&ocirc;le de la puissance
pendant et apr&egrave;s la prise de contact

- probl&egrave;mes de direction



Questions :

Au cours d'un atterrissage par vent de travers,
sur avion l&eacute;ger, on abaisse l'aile du c&ocirc;t&eacute;
du vent.  Pourquoi ?


Lorsque la piste d'atterrissage est soumise
&agrave; un vent fort de travers, nous devons consulter le graphique
des &#171;limites d'utilisation 

par vent de travers&#187; avan de nous d&eacute;cider
&agrave; utiliser cette piste.  Pourquoi ?


Si, au moment de l'impact, on exerce une pression
excessive sur la commande de profondeur d'un avion &agrave; train
tricycle effectuant un atterissage par vent de travers, un &#171;brouettage&#187;
peut en r&eacute;sulter.  Comment pouvons-nous emp&ecirc;cher
le &#171;brouettage&#187; ?


Quelles seront les cons&eacute;quences probables
du &#171;brouettage&#187; ?


Au cours d'une approche avec survol d'obstacle
en vue d'un atterrissage sur terrain court, on ne doit r&eacute;duire
compl&egrave;tement les gaz qu'apr&egrave;s avoir termin&eacute;
l'arrondi &agrave; l'atterrissage.  Pourquoi  ?


On doit absolument surveiller la vitesse au
cours d'une approche avec survol d'obstacles.  Une formule simple
permet de calculer cette vitesse.

a) Quelle est cette formule ?

b) Pourquoi faut-il surveiller la vitesse
?



En vol :


Equipement : visibilit&eacute; 5 milles

plafond  1500 pieds

r&egrave;gle ou crayon pour mesurer distances
angulaires


D&eacute;monstrations :

1) Atterrissages normaux :

+ <U>nommer</U> l'exercice

+ <U>d&eacute;finir</U> l'exercice

+ <U>motiver</U> l'&eacute;tudiant

+ <B>clef : seuil de piste &quot;immobile&quot;</B>

+ stabilisation sur l'axe d'approche en vue
de l'aterrissage (effet de           parallaxe)

+ contr&ocirc;le du taux de descente avec
volets et/ou puissance

+ indices visuels permettant de juger l'atterrissage
:

- garder le regard d&eacute;tendu

- regarder tout le panorama (hangars, avions
stationn&eacute;s, arbres,          herbe, horizon,....)

- pendant l'approche:  emp&ecirc;cher les
marques de <B>seuil</B> de piste de    &#171;bouger&#187; par
rapport au capot jusqu'au d&eacute;but de l'arrondi    (maintenir
vitesse constante avec gouv. de profondeur,     contr&ocirc;ler
taux de descente avec volets et puissance)

- d&eacute;but de l'arrondi :

effectuer rapidement (1) et (2) :

1) la hauteur &agrave; laquelle l'arrondi
<U>d&eacute;bute</U> et la <U>cadence</U> &agrave;    laquelle
il est effectu&eacute;, d&eacute;pendent du taux de descente 
   ant&eacute;rieur  (regarder &agrave; quelle vitesse les choses,
surtout    <B>proches du seuil</B>, &#171;bougent&#187;)

2) d&eacute;termination de la hauteur en
regardant sous quelle     perspective se pr&eacute;sentent des
objets familiers      (regarder surtout o&ugrave; <B>l'horizon
</B>&#171;coupe&#187; les objets)

3) diriger le regard <U>&agrave; gauche</U>
le long du capot, &agrave; la m&ecirc;me    distance qu'habituellement
d'une auto roulant &agrave; la m&ecirc;me    vitesse, c-a-d. <U>assez</U>
<U>loin</U> pour que le sol ne soit pas flou et    <U>assez pr&egrave;s</U>
pour pouvoir d&eacute;tecter les changements de     hauteur  
 (<U>environ</U>  <B>&agrave; mi-piste )</B>

- apr&egrave;s que la descente ait &eacute;t&eacute;
arr&ecirc;t&eacute;e  (en palier &agrave; environ 1') :

- une et une seule assiette pour chaque
hauteur au-dessus     de la piste

- d&eacute;terminer si l'avion s'enfonce
ou s'&eacute;l&egrave;ve le moindrement

(au sol,  s'exercer &agrave; d&eacute;tecter
les changements de     perspective en s'&eacute;levant sur le
bout des pieds)

- si l'avion s'&eacute;l&egrave;ve, simplement
attendre, garder la main     fixe, laisser le ballonnement se
dissipper par soi-    m&ecirc;me, puis recommencer &agrave; tirer
sur le manche      lorsque l'avion commence &agrave; s'enfoncer

- si l'avion s'enfonce, augmenter la pression
arri&egrave;re

- la vit. &agrave; laquelle on applique
la pression arri&egrave;re s'acc&eacute;-   l&egrave;re,  au fur
et &agrave; mesure que la vitesse de l'avion diminue

- v&eacute;rifier la direction <U>et l'inclinaison
lat&eacute;rale</U> au-dessus    de la piste, du coin de l'oeil,
et corriger avec les palonniers


2) Atterrissages par vent de travers :

+ <U>nommer</U> l'exercice

+ <U>d&eacute;finir</U> l'exercice

+ <U>motiver</U> l'&eacute;tudiant

+ <B>clef : correction de la d&eacute;rive
en finale</B>

+ v&eacute;rifier les recommendations du
manufacturier concernant    l'utilisation des volets pendant une
glissade

APPROCHE EN CRABE ET ATTERRISSAGE AILE BASSE
:

- pendant l'approche, corriger la d&eacute;rive
en maintenant un &#171;angle   de crabe&#187;

- s'il y a des rafales, ajouter &agrave;
la vitesse d'approche 1/2 gust

- au d&eacute;but de l'arrondi, adopter
et maintenir une glissade lat&eacute;rale   suffisante pour emp&ecirc;cher
tout d&eacute;placement lat&eacute;ral

- si le vent de travers est fort ou par
rafales,  prendre contact   avec la piste &agrave; une vitesse
sup&eacute;rieure &agrave; la normale pour conserver   l'efficacit&eacute;
du gouv. de direction et r&eacute;duire l'angle d'attaque (ne
  pas garder la roue en l'air apr&egrave;s s'&ecirc;tre pos&eacute;)

- pendant le cabrage final, le nez de l'avion
(l'axe longitudinal)   semblera prendre un angle avec l'axe de
piste du c&ocirc;t&eacute; oppos&eacute; au   vent &agrave; cause
de l'assiette simultan&eacute;ment cabr&eacute;e et inclin&eacute;e
   (d&eacute;montrer avec maquette)

- impact sur la roue principale au vent

- l'efficacit&eacute; des ailerons diminuant,
l'autre roue principale    touchera la piste

APPROCHE AILE BASSE ET ATTERRISSAGE AILE
BASSE :

- comme ci-dessus,  sauf que la d&eacute;rive
est corrig&eacute;e avec une    glissade lat&eacute;rale pendant
toute l'approche


3) Atterrissages courts  (obstacle de 50')
:

+ <U>nommer</U> l'exercice

+ <U>d&eacute;finir</U> l'exercice

+ <U>motiver</U> l'&eacute;tudiant

+ <B>clef : arrondi au seuil de piste</B>

+ approche <U>au moteur (</U>&agrave; vitesse
r&eacute;duite): &lt;1.3 Vso(CAS) + 1/2gust

+ faire un circuit plus large

+ au d&eacute;but de la finale :  pleins
volets et vitesse stabilis&eacute;e

contr&ocirc;le de la vitesse avec le manche

contr&ocirc;le de la pente avec la puissance

+ garder de la puissance tant que l'avion
n'est pas &eacute;tabli en palier au-  dessus de la piste  (i.e.
tant que l'arrondi n'est pas fini)

+ &eacute;viter de prolonger le flottement
au-dessus de la piste   (couper    compl&egrave;tement moteur
et placer l'avion au sol)

+ apr&egrave;s que le poser des roues,  diminuer
l'assiette,  rentrer les volets   (pas les roues !),  tenir le
manche compl&egrave;tement en arri&egrave;re et   freiner au maximum
sans bloquer les roues


4) Atterrissages sur terrain mou ou cahoteux
:

+ <U>nommer</U> l'exercice

+ <U>d&eacute;finir</U> l'exercice

+ <U>motiver</U> l'&eacute;tudiant

+ <B>clef : puissance pendant l'arrondi</B>

+ reconnaissance du terrain  (approche de
pr&eacute;caution :  ex. 21)

+ approche finale &quot;atter. court&quot;
(plus de pr&eacute;cision &agrave; la prise de contact)

+ garder de la puissance pendant tout l'arrondi
<U>et</U> jusqu'&agrave; ce que <U>toutes </U> <U>les roues</U>
de l'avion aient pris contact avec le sol, manche &agrave; fond
en  arri&egrave;re  (vit. min. de contact  roue avant)

+ prendre contact &agrave; la plus faible
vitesse verticale et horizontale  possible

+ ne pas utiliser les freins  (continuer
&agrave; rouler jusqu'&agrave; &ecirc;tre sur terrain   ferme)



5) Remise des gaz (approche manqu&eacute;e)
:

Jugement &agrave; d&eacute;velopper  :

- prendre la d&eacute;cision t&ocirc;t

- IMP :  une fois la puissance ajout&eacute;e,
ne jamais changer d'id&eacute;e

Technique :

- assiette, puissance, r&eacute;ch-carb,


- enlever dernier cran de volets

- trajectoire


6) pos&eacute;-d&eacute;coll&eacute; :

- contr&ocirc;le en direction

- utilisation des commandes auxiliaires

- piste restante suffisante


7) s&eacute;curit&eacute; sur la piste :

- virages &agrave; vitesse r&eacute;duite

- d&eacute;gagement de la piste en service

- v&eacute;rifications apr&egrave;s atterrissage

- autorisation de roulage


8) Lorsque les conditions s'y pr&ecirc;tent
en s&eacute;curit&eacute;, d&eacute;montrer l'influence sur la
distance d'atterrissage :

- du vent arri&egrave;re

- des surfaces en herbe, mouill&eacute;es
et s&egrave;ches

- de la pente de la piste

- du poids maximal &agrave; l'atterrissage




Normes de v&eacute;rification en vol
:

Approche et atterrissage :

- utilisation du moteur, volets et compensateur

- maintien de la vitesse

- trajectoire d'approche et correction de
la d&eacute;rive

- qualit&eacute; des atterrissages

- capacit&eacute; de se poser sur un point
pr&eacute;d&eacute;termin&eacute;

- tenue de l'axe de piste pendant le roulage



Remont&eacute;e :

- remise rapide et correcte des gaz

- utilisation correcte du r&eacute;chauffage
carburateur

- &eacute;tablissement d'un taux de mont&eacute;e
franc

- ma&icirc;trise de la vitesse et correction
du lacet

- &eacute;limination des tra&icirc;n&eacute;es
inutiles



Rapports d'accident :

Rapports de faits a&eacute;ronautiques, vol.
6


Aviation Safety's Flying Circus, 2/ed.


I learned about flying from that, p. 32


