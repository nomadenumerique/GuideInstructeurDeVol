# Exercice 8: Descente</h1>

Plans
de le&ccedil;ons    2,   3,   4



D&eacute;finition : manoeuvre
ex&eacute;cut&eacute;e pour passer d'un palier sup&eacute;rieur
&agrave; un    palier inf&eacute;rieur



Objectifs : +
mise en descente

+ descentes : - &agrave; diff&eacute;rentes
vitesses 

- avec ou sans puissance

- &agrave; diff&eacute;rentes configurations
          trains/volets

- avec ou sans obstacle

+ mise en palier &agrave; une altitude
sp&eacute;cifi&eacute;e



## Motivation :
manoeuvre fondamentale utilis&eacute;e &agrave; tous les vols.

La descente est utilis&eacute;e pendant l'une
des &eacute;tapes les plus importantes  d'un vol, soit l'approche
pour l'atterrissage (cela peut &ecirc;tre un  atterrissage forc&eacute;).
 On peut aussi y avoir recours pour se soustraire &agrave;  de
forts vents contraires, pour passer en bas d'une couche nuageuse,
 pour mieux profiter du r&eacute;gime d'autonomie maximale,...




R&eacute;f&eacute;rences :
GIV 87

MP 70

MV 188-224

PPI 62-69

PPII 223-232  (volets)

S&amp;R 28-31;  41-42;  240-260;  263-277;
 284-286

Van Sickle 389-390; 393; 395-396; 399

AOPA Flt Training  8-1 &agrave; 8-20

FGU 26

Kershner SPFM  63-65; 78

Kershner APM 33-34; 106-112

Kershner FIM  28, 34, 42-44



Au sol :

Aides : + maquette d'avion

+ Manuel de l'avion 1-5; 2-15 &agrave; 2-16;
6-5

POH  3-9 &agrave; 3-11; 4-8


Points &agrave; d&eacute;montrer :

+ graphique des <U>forces</U>

+ <U>polaire</U> d'un profil (comment la
finesse varie avec l'angle          d'attaque, ou vitesse)

+ <U>finesse maximum</U>   (distance parcourue
max. par vent nul)

+ influence du <U>vent</U> sur la pente
et sur le taux de descente

+ <U>taux de chute minimum</U>  (utilis&eacute;
lorsqu'on d&eacute;sire accentuer             angle de descente
contre le vent)

+ descente <U>sans puissance</U> :

- utilisations :  pour perdre rapidement
de l'altitude

- &agrave; vitesse de plan&eacute; optimal
(et mani&egrave;re d'estimer la     distance que peut parcourir
l'avion)

- aux vitesses sup&eacute;rieures ou inf&eacute;rieures
&agrave; la vitesse de    plan&eacute; optimal (augmentation de
pente)

- diminution de l'efficacit&eacute; de
la gouverne de profondeur,    d&ucirc;e &agrave; la perte du souffle
h&eacute;lico&iuml;dal

+ descente <U>au moteur</U> :

- avantages :  &eacute;conomie, meilleur
contr&ocirc;le de la pente,       refroidissement uniforme,...

- &agrave; diff&eacute;rentes vitesses

- en route

+ descentes avec franchissement d'<U>obstacle</U>

+ <U>technique de mise en descente</U> (3
&eacute;tapes centrales):

- v&eacute;rifications cabine (raison d'&ecirc;tre)

- surveillance ext&eacute;rieure

- d&eacute;terminer vitesse de descente

(1) r&eacute;ch carburateur

(2) puissance r&eacute;duite

(3) maintenir assiette de croisi&egrave;re

- contr&ocirc;ler le lacet

- &agrave; la vitesse, assiette piqu&eacute;e
approximative

- maintenir avec palonnier axe de descente
(bille centr&eacute;e)

- ajustements mineurs de l'assiette

- compenser

- surveiller instruments moteur

+ <U>technique de mise en palier</U> (3
&eacute;tapes centrales, mais **pas** dans           le m&ecirc;me
ordre):

- d&eacute;terminer l'altitude du palier
et la vitesse (avec sa           puissance requise)

- &agrave; 10% du VSI pr&eacute;c&eacute;dant
l'altitude,

(1) appliquer puissance inf&eacute;rieure
de 100 RPM &agrave; celle     requise 

simultan&eacute;ment (2) prendre peu &agrave;
peu l'assiette de croisi&egrave;re 

(3) enlever r&eacute;ch carb

- contr&ocirc;ler lacet

- ajustements mineurs

- compenser

- v&eacute;rifications cabine


+ <U>lacet </U>:   - d&ucirc; &agrave; l'absence
du souffle h&eacute;lico&iuml;dal (d&eacute;calage de la    d&eacute;rive,
augmentation de la correction avec augmentation de    la vitesse)

-  lors de la remise des gaz

+ raisons d'&ecirc;tre et techniques de
<U>r&eacute;chauffage du moteur</U> lors     de descentes prolong&eacute;es
sans puissance (trois raisons)

+ utilisation du <U>r&eacute;chauffage-carburateur
 </U>(voir POH)


+ indications visuelles pour d&eacute;tecter
le <U>point d'impact</U> (TRES IMP)

+ indications des <U>instruments</U>

+ descendre en virant

+ utilisation des <U>volets</U> : _ r&eacute;duction
de la vitesse de d&eacute;crochage

(mais diminution de l'angle critique
max)

_ augmentation de la pente

_ changement d'assiette (sh&eacute;ma
CL
vs &aring;)

+ effet de la sortie du <U>train escamotable</U>

+ <U>puissance = taux de descente</U> (altitude)

<U>assiette = vitesse</U>

L'avion prend de la vitesse lorsque la puissance
est r&eacute;duite et   aucune pression n'est exerc&eacute;e sur
le manche &agrave; cause de la perte   du souffle h&eacute;lico&iuml;dal
sur la gouverne de profondeur et de la    diminution du couple
traction-tra&icirc;n&eacute;e  (c'est un facteur de    s&eacute;curit&eacute;).
Th&eacute;oriquement la vitesse devrait rester la m&ecirc;me,
si on   ne change pas la compensation.



Discipline a&eacute;ronautique :

- compl&eacute;ter la liste de v&eacute;rification
(pour ne pas oublier d'enrichir le  m&eacute;lange et d'utiliser
le r&eacute;chauffage carburateur en bas de 2000 RPM))

- obtenir le calage altim&egrave;trique le
plus r&eacute;cent

- s'assurer que le moteur ne se refroidisse
pas trop brutalement

- v&eacute;rifier que l'espace soit d&eacute;gag&eacute;
et maintenir une bonne surveillance  ext&eacute;rieure (virer
&agrave; tous les 500' si on a une assiette peu piqu&eacute;e)

- ne pas trop s'approcher du sol (1000' ds
un rayon de 2000' des  obstacles dans une agglom&eacute;ration
et 500'/sol en campagne)

- &ecirc;tre conscient de la nature de l'espace
a&eacute;rien en-dessous (&quot;C&quot;,  a&eacute;roports,...)

- r&eacute;chauffer le moteur &agrave; tous
les 200' (2000 RPM pendant 3 secondes) - manipuler la manette
des gaz avec souplesse

- anticiper les effets des changements de
puissance (garder l'assiette  en tangage d&eacute;sir&eacute;e
et emp&ecirc;cher le lacet)

- s'assurer d'&ecirc;tre en bas de Vfe avant
de sortir les volets

- s'assurer d'&ecirc;tre en haut de Vs**1**
avant de remonter les volets

- utiliser les volets par &eacute;tapes de
10&#176; et compenser apr&egrave;s chaque &eacute;tape

- exercer un contr&ocirc;le ferme, positif
et souple sur l'avion



R&eacute;capitulation :

Erreurs les plus courantes :

- probl&egrave;mes de direction : ne pas
tenir le nez align&eacute; sur le point de  r&eacute;f&eacute;rence
(trop d'attention sur l'assiette et la vitesse au d&eacute;triment
de  l'inclinaison ou du lacet)

- ne pas attendre suffisament longtemps (10
sec.) que le r&eacute;ch-carb  agisse avant de r&eacute;duire
la puissance

- manipulation brutale de la manette des
gaz

- laisser le nez monter apr&egrave;s avoir
adopt&eacute; l'assiette correcte (vitesse            diminue)

- vitesse chute &agrave; chaque fois que
les volets sont ajout&eacute;s

- garder le nez trop bas lorsque les volets
sont rentr&eacute;s (vitesse            augmente)

- oublier de d&eacute;crasser r&eacute;guli&egrave;rement
le moteur (hiver comme &eacute;t&eacute;)

- lorsque c'est l'&eacute;tudiant qui met
les volets, il est en arri&egrave;re de l'avion   (n&eacute;glige
assiette en tangage et direction)

- sur-corriger ou sous-corriger le lacet
pendant la transition

- transition brutale

- fixation sur l'an&eacute;mom&egrave;tre

- courir apr&egrave;s l'aiguille de l'an&eacute;mom&egrave;tre
(en ne gardant pas l'assiette  fixe suffisament longtemps)



Questions :

Si, en descendant &agrave; vitesse constante,
nous sortons les volets, la vitesse verticale augmente. Pourquoi
?


La formule &#171;1.3 &agrave; 1.4 fois la
vitesse corrig&eacute;e de d&eacute;crochage moteur r&eacute;duit&#187;
est utilis&eacute;e pour calculer la vitesse d'approche finale
au cours d'une approche au-dessus des obstacles. Quelle vitesse
indiqu&eacute;e utiliserons-nous en descendant selon cette formule
&agrave; bord de notre avion habituel ?


Au cours d'une descente moteur r&eacute;duit,
nous devons r&eacute;guli&egrave;remennt augmenter les gaz.  Quelle
en est la raison ?


Au cours d'une descente moteur r&eacute;duit
ex&eacute;cut&eacute;e rigoureusement face au vent nous pouvons
augmenter la distance franchissable en augmentant la vitesse.
Quelle en est la raison ?


Au cours d'une descente &agrave; moteur r&eacute;duit,
nous avons sur cet avion &agrave; exercer une pression sur le
palonnier gauche pour pouvoir rester en ligne droite. Pourquoi
?


Quel est l'effet du vent sur le taux de descente
?  Sur l'angle de descente ?

Sur la distance plan&eacute;e ?


Dans quelle situation lors d'une panne moteur
utiliserait-on la vitesse de <U>taux </U>de chute minimum ?


Pourquoi enrichit-on le m&eacute;lange avant
d'amorcer une descente ?


Quel est le r&eacute;glement concernant l'altitude
minimale par rapport au sol ?



En vol :

Equipement : normal et horizon

r&egrave;gle ou crayon pour mesurer distances
angulaires


D&eacute;monstration principale :

+ <U>nommer</U> l'exercice

+ <U>d&eacute;finir</U> l'exercice

+ <U>motiver</U> l'&eacute;tudiant


+ <U>**clef**</U>**  : <U>puissance,
assiette  et  compensateur</U>   (PAT)**

+ <U>technique de mise en descente</U> (3
&eacute;tapes centrales):

- v&eacute;rifications cabine (raison d'&ecirc;tre)

- surveillance ext&eacute;rieure

- d&eacute;terminer vitesse de descente

(1) r&eacute;ch carburateur

(2) puissance r&eacute;duite

(3) maintenir assiette de croisi&egrave;re

- contr&ocirc;ler le lacet

- &agrave; la vitesse, assiette piqu&eacute;e
approximative

- maintenir avec palonnier axe de descente

- ajustements mineurs

- compenser

- surveiller instruments moteur

+ <U>technique de mise en palier</U> (3
&eacute;tapes centrales, mais **pas** dans           le m&ecirc;me
ordre):

- d&eacute;terminer l'altitude du palier
et la vitesse et puissance            requises

- &agrave; 10% du VSI pr&eacute;c&eacute;dant
l'altitude,

(1) appliquer puissance inf&eacute;rieure
de 100 RPM &agrave; celle     requise 

simultan&eacute;ment (2) prendre peu &agrave;
peu l'assiette de croisi&egrave;re 

(3) enlever r&eacute;ch carb

- contr&ocirc;ler lacet

- ajustements mineurs

- compenser

- v&eacute;rifications cabine


D&eacute;montrer aussi :

+ indications des <U>instruments</U>

+ <U>lacet</U> : d&ucirc; &agrave; l'absence
du souffle h&eacute;lico&iuml;dal

lors de la remise des gaz

+ <U>r&eacute;chauffage</U> <U>du moteur</U>

+ utilisation du <U>r&eacute;chauffage-carburateur</U>

+ indications visuelles pour d&eacute;tecter
le <U>point d'impact</U>

+ utilisation des <U>volets</U> : _ r&eacute;duction
de la vitesse de d&eacute;crochage

(mais diminution de l'angle critique
max)

_ augmentation de la pente

_ changement d'assiette (sh&eacute;ma
CL
vs &aring;)

+ influence du <U>vent</U> sur la pente
et sur le taux de descente

+ effet de la sortie du <U>train escamotable</U>

+ <U>puissance = taux de descente</U>

<U>assiette = vitesse</U>

L'avion prend de la vitesse lorsque la puissance
est r&eacute;duite et   aucune pression n'est exerc&eacute;e sur
le manche &agrave; cause de la perte   du souffle h&eacute;lico&iuml;dal
sur la gouverne de profondeur (c'est un    facteur de s&eacute;curit&eacute;).
Th&eacute;oriquement la vitesse devrait rester la   m&ecirc;me,
si on ne change pas la compensation.


+ <U>descente en route</U>

+ descente avec franchissement d'<U>obstacle</U>
(descente au moteur             avant l'obstacle)


<U>Faire pratiquer &agrave; diff&eacute;rentes
vitesses</U>

<U>Faire pratiquer avec volets</U>
 (pour commencer, c'est l'instructeur qui met les      volets)

<U>Faire pratiquer en masquant l'an&eacute;mom&egrave;tre</U>
 (contr&ocirc;le par r&eacute;f&eacute;rence &agrave; l'assiette
        et &agrave; la puissance seulement)



Normes de v&eacute;rification en vol
:

# Exercice  :

(1) d&eacute;part &agrave; 4000' AGL

(2) vol rect. en palier 1 min. (&#177; 50')

(3) mont&eacute;e 4500'

(4) vol rect. en palier 1 min. (&#177; 50')

(5) descente 4000'

(6) vol rect. en palier 1 min. (&#177; 50')

(7) vir. 90&#176; en mont&eacute;e &agrave;
gauche, &agrave; droite, jusqu'&agrave; 5000'

(8) vol rect. en palier 1 min. (&#177; 50')

(9) vir. 90&#176; en descente &agrave; gauche,
&agrave; droite, jusqu'&agrave; 4000'


Vol  :  coordonn&eacute;

Cap :  &#177; 10 &#176;

Vitesse : &#177; 10 kts



Rapports d'accident :

Rapports de faits a&eacute;ronautiques, vol.
6


Aviation Safety's Flying Circus, 2/ed.


I learned about flying from that
